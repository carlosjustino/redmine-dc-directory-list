class SocketprogressController < ApplicationController
  ...
  def invocaServicoProgress()
     flash[:notice] = "Buscando arquivos no servidor do Progress..."
	   porta =  getNewPortSocket()
     servico = "FontesDifCliente"
     xmlParam =  montaXmlParametros()
  ...
  def processaRetornoErro(xmlErro)
     puts(xmlErro)
     flash[:error] = xmlErro.xpath("//res:descricao").text()
